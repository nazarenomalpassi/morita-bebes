...
export function AnalyticsDashboard({ report, period, search }: { report: AnalyticsReport; period: ResolvedReportPeriod; search: ReportSearchParams }) {
  ...
  const kpis = [
    ...
    { label: "Gastos", value: money.format(current.operatingExpenses), detail: current.expenseToRevenuePct === null ? "Sin relación calculable" : `${percent.format(current.expenseToRevenuePct)}% de las ventas`, icon: ReceiptText, current: current.operatingExpenses, previous: previous.operatingExpenses },
