...
export default async function PersonnelPage({
  ...
  const [employeesResult, movementsResult, dashboardResult, settlementsResult, membersResult, paymentMethodsResult] = await Promise.all([
    ...
    supabase
      ...
      .select("*, employees(first_name, last_name), expenses(expense_date, description, payment_method_id), payment_methods(name, code)")
          ...
          <div className="compact-list">
            {adjustmentMovements.filter((movement) => !selectedEmployeeId || movement.employee_id === selectedEmployeeId).map((movement) => (
              <div className="compact-list-row" key={movement.id}>
                <div><strong>{movement.employees?.last_name}, {movement.employees?.first_name}</strong><small>{kindLabels[movement.kind]} · {movement.expense_id ? `Registro histórico del ${localDate(movement.expenses?.expense_date ?? movement.period_month)}` : movement.paid_at ? localDate(movement.paid_at) : "Fecha sin registrar"} · {movement.payment_methods?.name ?? (movement.expenses?.payment_method_id ? paymentNameById.get(movement.expenses.payment_method_id) : null) ?? (movement.expense_id ? "Medio histórico no disponible" : "Sin caja asociada (histórico)")}</small>{movement.voided_at ? <small>Anulado: {movement.void_reason}</small> : null}</div>
