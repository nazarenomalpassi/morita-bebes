...
export default async function ExpensesPage({ searchParams }: { searchParams: Promise<{ mes?: string; empleado?: string }> }) {
  ...
  let expensesQuery = supabase.from("expenses").select("*, expense_categories(name, is_payroll_advance), payment_methods(name), payroll_employee:employees!expenses_payroll_employee_fk(first_name, last_name)").eq("organization_id", organization.id).gte("expense_date", `${month}-01`).lt("expense_date", nextMonth).order("expense_date", { ascending: false }).order("created_at", { ascending: false });
  if (employee && organization.role !== "staff") expensesQuery = expensesQuery.eq("created_by", employee);

  const [expensesResult, categoriesResult, methodsResult, membersResult, ownProfileResult, employeesResult] = await Promise.all([
    expensesQuery,
    ...
    organization.role === "staff" ? Promise.resolve({ data: [], error: null }) : supabase.from("employees").select("id, first_name, last_name").eq("organization_id", organization.id).eq("status", "active").order("last_name"),
  ]);
  if (expensesResult.error || categoriesResult.error || methodsResult.error || membersResult.error || ownProfileResult.error || employeesResult.error) throw new Error("No se pudo cargar la información de gastos.");

  const expenses = expensesResult.data ?? [];
  const categories = categoriesResult.data ?? [];
  const paymentMethods = methodsResult.data ?? [];
  const payrollEmployees = (employeesResult.data ?? []).map((item) => ({ id: item.id, name: `${item.last_name}, ${item.first_name}` }));
  ...
  return (
    <div className="page-container page-container-wide">
      <header className="page-header"><div><span className="eyebrow">Egresos operativos</span><h1 className="page-title mt-2">Gastos</h1><p className="page-lead">Cada registro conserva autor, fecha real y estado.</p></div><details className="header-details"><summary className="button button-primary"><Plus size={17} /> Nuevo gasto</summary><div className="details-panel"><ExpenseForm canAssignPayrollAdvance={organization.role !== "staff"} canChooseDate={organization.role !== "staff"} categories={categories} employees={payrollEmployees} paymentMethods={paymentMethods} /></div></details></header>
