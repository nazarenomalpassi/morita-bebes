...
export default async function ExpensesPage({ searchParams }: { searchParams: Promise<{ mes?: string; empleado?: string }> }) {
  ...
  const [expensesResult, categoriesResult, methodsResult, membersResult, ownProfileResult] = await Promise.all([
    expensesQuery,
  ...
  if (expensesResult.error || categoriesResult.error || methodsResult.error || membersResult.error || ownProfileResult.error) throw new Error("No se pudo cargar la información de gastos.");
  ...
  return (
    <div className="page-container page-container-wide">
      <header className="page-header"><div><span className="eyebrow">Egresos operativos</span><h1 className="page-title mt-2">Gastos</h1><p className="page-lead">Cada registro conserva autor, fecha real y estado.</p></div><details className="header-details"><summary className="button button-primary"><Plus size={17} /> Nuevo gasto</summary><div className="details-panel"><ExpenseForm canChooseDate={organization.role !== "staff"} categories={categories} paymentMethods={paymentMethods} /></div></details></header>
